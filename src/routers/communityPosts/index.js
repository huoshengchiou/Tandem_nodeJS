...
router.post("/uploaditem", upload.single("communityImage"), (req, res) => {
  ...
  const sql = `INSERT INTO \`posts\`(\`postTitle\`, \`postContent\`,  \`postImg\`,\`postTag\`,\`postViews\`,\`postComments\`,\`member_id\`) 
    VALUES (?, ?, ?,?,?,?,?)`;
...
router.get("/posts", (req, res) => {
  ...
  router.post("/postCategory", (req, res) => {
    ...
    if (req.body.pCategory_id) {
      ...
      db.queryAsync(category_sql, [req.body.searchData, req.body.searchData])
